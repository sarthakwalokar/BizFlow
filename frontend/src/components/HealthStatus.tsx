  ...
  const checkHealth = async () => {
    ...
    try {
      ...
      if (isTimeout) {
        ...
        setError(t('health.coldStartMsg', 'Backend is waking up from idle state. Please allow a moment for instance initialization.'));
      } else {
        setIsColdStarting(false);
        setError(errMsg || t('health.connectError', 'Unable to connect to backend service. Please check connectivity.'));
      }
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-base font-bold text-zinc-900 tracking-tight flex items-center gap-2">
              {t('health.title', 'System Health & Diagnostics')}
              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold ${
                ...
                {loading ? t('health.checking', 'Checking...') : isColdStarting ? t('health.wakingUp', 'Waking Up') : isUp ? t('health.operational', 'Operational') : t('health.degraded', 'Degraded')}
              </span>
            </h2>
            <p className="text-xs text-zinc-500 mt-0.5">
              {t('health.subtitle', 'Real-time status of Spring Boot REST backend and PostgreSQL database')}
            </p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-zinc-50/70 border border-zinc-200/80 space-y-1.5">
          <div className="flex items-center justify-between text-xs font-medium text-zinc-500">
            <span className="flex items-center gap-1.5">
              ...
              {t('health.apiService', 'API Service')}
            </span>
            {isUp ? (
          ...
          <div className="text-sm font-bold text-zinc-900">
            {isUp ? 'Spring Boot 3.3.4 (UP)' : isColdStarting ? t('health.startingUp', 'Starting Up / Waking') : t('health.offline', 'Service Offline')}
          </div>
          <div className="text-[11px] text-zinc-400">
            {isColdStarting ? t('health.booting', 'Backend instance booting...') : latencyMs !== null ? `${t('health.latency', 'Latency')}: ${latencyMs}ms` : `${t('health.latency', 'Latency')}: —`}
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-zinc-50/70 border border-zinc-200/80 space-y-1.5">
          <div className="flex items-center justify-between text-xs font-medium text-zinc-500">
            <span className="flex items-center gap-1.5">
              ...
              {t('health.databaseEngine', 'Database Engine')}
            </span>
            {diagnosticsLoading ? (
          ...
          <div className="text-[11px] text-zinc-400">
            {diagnosticsLoading ? t('health.inspecting', 'Inspecting connectivity...') :
             health?.database === 'CONNECTED' ? t('health.connected', 'Connection: CONNECTED') :
             health?.database === 'DISCONNECTED' ? t('health.disconnected', 'Connection: DISCONNECTED') :
             isColdStarting ? t('health.wakingWithBackend', 'Waking with backend') :
             isUp ? t('health.ready', 'Connection: READY') :
             t('health.disconnected', 'Connection: DISCONNECTED')}
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-zinc-50/70 border border-zinc-200/80 space-y-1.5">
          <div className="flex items-center justify-between text-xs font-medium text-zinc-500">
            <span className="flex items-center gap-1.5">
              ...
              {t('health.lastInspected', 'Last Inspected')}
            </span>
          </div>
          <div className="text-sm font-bold text-zinc-900">
          ...
          <div className="text-[11px] text-zinc-400">
            {t('health.autoValidated', 'Auto-validated on load')}
          </div>
        </div>
      </div>
      ...
      {error && (
        <div className={`mt-4 p-3.5 rounded-xl border text-xs flex items-start space-x-2.5 ${
          ...
          <div className="space-y-0.5">
            <p className="font-semibold">{isColdStarting ? t('health.coldStartTitle', 'Instance Cold Start in Progress') : t('health.diagnosticsNotice', 'Diagnostics Notice')}</p>
