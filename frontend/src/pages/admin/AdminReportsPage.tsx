    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        <div className="p-4 rounded-xl bg-white border border-zinc-200 shadow-xs space-y-1">
          ...
          <span className="text-[11px] text-purple-700 font-medium block">100% platform coverage</span>
        </div>
        ...
        <div className="p-4 rounded-xl bg-white border border-zinc-200 shadow-xs space-y-1">
          ...
          <div className="text-xl font-bold text-purple-700">{report?.largeBusinessesCount ?? 0}</div>
          <span className="text-[11px] text-purple-600 block">Multi-location inventory enabled</span>
        </div>
      </div>
          ...
          {trendData.map((pt, idx) => {
            ...
            return (
              <div key={idx} className="flex-1 flex flex-col items-center gap-1.5 group h-full justify-end">
                <span className="text-[10px] font-semibold text-purple-700 opacity-0 group-hover:opacity-100 transition-opacity">
                  {pt.registrations}
                </span>
                <div
                  className="w-full max-w-[40px] bg-purple-600 hover:bg-purple-700 rounded-t-md transition-all"
              ...
              return (
                <div key={type} className="p-3.5 rounded-lg bg-zinc-50 border border-zinc-200 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-mono font-semibold text-purple-700">{pct}%</span>
                  </div>
